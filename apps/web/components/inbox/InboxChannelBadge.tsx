...
const CHANNEL_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  webchat: {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full" aria-hidden="true">
        <path d="M12 2C6.477 2 2 6.477 2 12c0 1.821.487 3.53 1.338 5L2.5 21.5l4.5-.838A9.955 9.955 0 0 0 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2z" />
      </svg>
    ),
  },
  email: {
...
export function InboxChannelBadge({ channel, size = "sm" }: InboxChannelBadgeProps) {
  const config = CHANNEL_CONFIG[channel] ?? {
    label: channel,
    color: "#6B7280",
    icon: <MessageSquare className="w-full h-full" aria-hidden="true" />,
  };
  ...
  return (
    <span
      className={`inline-flex items-center justify-center ${sizeClass} flex-shrink-0`}
